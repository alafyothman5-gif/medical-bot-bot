...
async def process_job(bot: Bot, job: dict):
    ...
    if job_type == "summary":
        from bot import get_or_create_summary, db_add_xp, set_ai_user_id
    ...
    if job_type == "preload_file":
        from bot import get_or_create_summary, get_or_create_question_bank, SUMMARY_PROMPTS, LEVEL_LIMITS, set_ai_user_id

        set_ai_user_id(user_id)
        file_hash = payload.get("file_hash", "")
        text = payload.get("text", "")
        file_name = payload.get("file_name", "lecture")
        styles = payload.get("styles") or list(SUMMARY_PROMPTS.keys())
        levels = payload.get("levels") or list(LEVEL_LIMITS.keys())

        await bot.send_message(chat_id=chat_id, text=f"📚 بدأ تجهيز كاش محاضرة الفصل في الخلفية...\n📄 {file_name}\nJob ID: {job_id}")
        made_summaries = 0
        made_banks = 0
        for style in styles:
            try:
                ...
                logger.exception("preload summary failed style=%s", style)
        for level in levels:
            try:
                questions = await get_or_create_question_bank(file_hash, text, level)
                if questions:
                    made_banks += 1
            except Exception:
                logger.exception("preload questions failed level=%s", level)

        await bot.send_message(
            chat_id=chat_id,
            text=(
                "✅ انتهى تخزين محاضرة الفصل.\n"
                f"📄 الملف: {file_name}\n"
                f"📚 ملخصات جاهزة: {made_summaries}\n"
                f"📝 بنوك أسئلة جاهزة: {made_banks}\n"
                "أي طالب يرفع نفس الملف/نفس النص سيستخدم الكاش بدون API."
            ),
        )
        set_job_status(job_id, "done", result={"ok": True, "summaries": made_summaries, "banks": made_banks})
        return

    if job_type == "question_bank":
        from bot import get_or_create_question_bank, normalize_questions, set_ai_user_id
