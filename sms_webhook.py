...
def _normalize_sms_sender(value: str) -> str:
    """Normalize sender name/number for safe comparison."""
    return "".join(str(value or "").strip().lower().split())


def _sender_allowed(sender: str) -> bool:
    """
    If ALLOWED_SMS_SENDERS is empty: allow and just log sender.
    If set: only allow exact normalized sender match.
    Example .env:
    ALLOWED_SMS_SENDERS=Almadar,المدار
    """
...
async def sms_handler(request: web.Request) -> web.Response:
    ...
    # No exact pending order: store it for 24h so a later student order can be linked automatically.
    try:
        await bot.db_store_unmatched_madar_sms(phone, float(amount), sms_text)
    except Exception:
        pass
