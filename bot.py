...
PAST_PAPER_FREE_DAILY_LIMIT = int(os.environ.get("PAST_PAPER_FREE_DAILY_LIMIT", "20"))
PAST_PAPER_USAGE_KIND = "past_paper_question"
MADAR_PAYMENT_NUMBER = os.environ.get("MADAR_PAYMENT_NUMBER", os.environ.get("ALMADAR_PHONE", "0918874659")).strip()
SMS_WEBHOOK_TOKEN = os.environ.get("SMS_WEBHOOK_TOKEN", "MadarPay2026").strip()
PAYMENT_ORDER_TTL_SECONDS = int(os.environ.get("PAYMENT_ORDER_TTL_SECONDS", "7200"))
SMS_DUPLICATE_TTL_DAYS = float(os.environ.get("SMS_DUPLICATE_TTL_DAYS", "14") or 14)
PAYMENT_SMS_RETENTION_DAYS = int(os.environ.get("PAYMENT_SMS_RETENTION_DAYS", "30") or 30)
PAYMENT_UNCLAIMED_MATCH_SECONDS = int(os.environ.get("PAYMENT_UNCLAIMED_MATCH_SECONDS", "86400") or 86400)
PAYMENT_ALLOWED_ORDER_AGE_SECONDS = int(os.environ.get("PAYMENT_ALLOWED_ORDER_AGE_SECONDS", str(PAYMENT_ORDER_TTL_SECONDS)) or PAYMENT_ORDER_TTL_SECONDS)
PLAN_PRICES_AR = {
    "premium_month": "20 د.ل / شهر",
    "premium_semester": "45 د.ل / فصل كامل 4 شهور",
    "premium_plus_semester": "65 د.ل / فصل كامل 4 شهور",
}
PLAN_USAGE_LIMITS = {
    "free": {
...
def init_db() -> None:
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS payment_orders (
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS user_daily_usage (
        user_id INTEGER NOT NULL,
        day TEXT NOT NULL,
        kind TEXT NOT NULL,
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS unmatched_madar_sms (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        phone TEXT NOT NULL,
        amount REAL NOT NULL,
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS unclaimed_madar_sms (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        claimed_order_id INTEGER DEFAULT 0,
        claimed_user_id INTEGER DEFAULT 0,
        claimed_at REAL DEFAULT 0,
        note TEXT DEFAULT ''
    )""")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_unclaimed_madar_sms_status ON unclaimed_madar_sms(status, received_at)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_unclaimed_madar_sms_match ON unclaimed_madar_sms(parsed_phone, parsed_amount, status, received_at)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_payment_orders_pending ON payment_orders(status, payer_phone, amount, expires_at)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_payment_orders_user_status ON payment_orders(user_id, status, created_at)")
    for alter_sql in [
...
async def db_get_daily_usage(user_id: int, kind: str, day: Optional[str] = None) -> int:
    day = day or today_local_key()
    def work():
        conn = _db_connect()
        row = conn.execute(
            "SELECT count FROM user_daily_usage WHERE user_id=? AND day=? AND kind=?",
            (int(user_id), day, kind),
        ).fetchone()
        conn.close()
...
async def db_increment_daily_usage(user_id: int, kind: str, amount: int = 1, day: Optional[str] = None) -> int:
    day = day or today_local_key()
    amount = max(0, int(amount or 0))
    if amount <= 0:
        return await db_get_daily_usage(user_id, kind, day)
    async with _db_lock:
        def work():
            conn = _db_connect()
            row = conn.execute(
            ...
            current = int(row["count"] or 0) if row else 0
            new_count = current + amount
            conn.execute(
                "INSERT OR REPLACE INTO user_daily_usage(user_id, day, kind, count, updated_at) VALUES(?,?,?,?,?)",
                (int(user_id), day, kind, new_count, time.time()),
            )
            conn.commit(); conn.close()
            return new_count
        return await asyncio.to_thread(work)
...
def normalize_madar_phone(phone: str) -> str:
    """Normalize Almadar numbers: 21891/21893 -> 091/093 and keep 091/093."""
    digits = "".join(ch for ch in str(phone or "") if ch.isdigit())
    if digits.startswith("00218"):
        digits = digits[2:]
    if digits.startswith("21891") and len(digits) >= 12:
        return "0" + digits[3:12]
    if digits.startswith("21893") and len(digits) >= 12:
        return "0" + digits[3:12]
    if digits.startswith("91") and len(digits) == 9:
        return "0" + digits
    if digits.startswith("93") and len(digits) == 9:
        return "0" + digits
    if (digits.startswith("091") or digits.startswith("093")) and len(digits) >= 10:
        return digits[:10]
    return digits


def normalize_sms_sender(sender: str) -> str:
    return "".join(str(sender or "").strip().lower().split())


def extract_madar_transfer_sms(text: str) -> dict:
    """Extract transfer amount + sender phone from Almadar SMS text."""
    raw = str(text or "")
    cleaned = raw.replace("٫", ".").replace("،", " ").replace(",", ".")
    amount_area = re.split(r"رصيدك\s+الحالي|رصيدك الحالي|balance", cleaned, maxsplit=1, flags=re.I)[0]
    amount_matches = re.findall(r"(\d+(?:\.\d+)?)\s*(?:د\.?ل|دينار|LYD|LD)", amount_area, flags=re.I)
    if not amount_matches:
        amount_matches = re.findall(r"(?:تحويل|حول|استلام|received|transfer)\D{0,30}(\d+(?:\.\d+)?)", amount_area, flags=re.I)
    if not amount_matches:
        amount_matches = re.findall(r"(?:amount|amt)\D{0,12}(\d+(?:\.\d+)?)", amount_area, flags=re.I)
    amount = None
    if amount_matches:
        try:
            ...
            amount = None
    phone_matches = re.findall(r"(?:00218(?:91|93)\d{7}|218(?:91|93)\d{7}|0(?:91|93)\d{7}|(?:91|93)\d{7})", cleaned)
    phone = normalize_madar_phone(phone_matches[0]) if phone_matches else ""
    return {"amount": amount, "phone": phone, "raw": raw}


def _payment_order_code() -> str:
    return "PAY-" + uuid.uuid4().hex[:8].upper()


def _payment_expiry_for_plan(plan: str, days: int) -> float:
    if (plan or "").strip().lower() in {"premium", "premium_plus"}:
        return time.time() + max(1, int(days or 30)) * 86400
    return 0.0


def _set_user_plan_in_conn(conn: sqlite3.Connection, user_id: int, plan: str, days: int) -> None:
    plan = (plan or "free").strip().lower()
    if plan not in {"free", "premium", "premium_plus", "admin"}:
        plan = "free"
    expires = _payment_expiry_for_plan(plan, int(days or 0)) if plan in {"premium", "premium_plus"} else 0.0
    now_iso = datetime.utcnow().isoformat()
    conn.execute(
        "INSERT INTO users(user_id, plan, plan_expires_at, created_at, last_seen) VALUES(?,?,?,?,?) "
        "ON CONFLICT(user_id) DO UPDATE SET plan=excluded.plan, plan_expires_at=excluded.plan_expires_at, last_seen=excluded.last_seen",
        (int(user_id), plan, float(expires), now_iso, now_iso),
    )


def payment_plan_details(plan_code: str, user_id: int) -> Optional[dict]:
    plans = {
        "premium_month": {"title": "Premium شهري", "amount": 20.0, "plan": "premium", "days": 30},
        "premium_semester": {"title": "Premium فصل كامل / 4 شهور", "amount": 45.0, "plan": "premium", "days": 120},
        "premium_plus_semester": {"title": "Premium Plus فصل كامل / 4 شهور", "amount": 65.0, "plan": "premium_plus", "days": 120},
    }
    return plans.get(plan_code)


async def db_get_open_pending_payment_order(user_id: int) -> Optional[dict]:
    now = time.time()
    def work():
        conn = _db_connect()
        conn.execute("UPDATE payment_orders SET status='expired', note='auto expired' WHERE status='pending' AND expires_at < ?", (now,))
        row = conn.execute(
            "SELECT * FROM payment_orders WHERE user_id=? AND status='pending' AND expires_at>? ORDER BY created_at DESC LIMIT 1",
            (int(user_id), now),
        ).fetchone()
        conn.commit(); conn.close()
        return dict(row) if row else None
...
async def db_cancel_pending_payment_order(user_id: int, note: str = "cancelled by user") -> int:
    now = time.time()
    async with _db_lock:
        def work():
            conn = _db_connect()
            cur = conn.execute(
                "UPDATE payment_orders SET status='cancelled', matched_at=?, note=? WHERE user_id=? AND status='pending'",
                (now, note, int(user_id)),
            )
...
async def db_create_payment_order(user_id: int, plan: str, days: int, plan_title: str, amount: float, payer_phone: str) -> dict:
    payer_phone = normalize_madar_phone(payer_phone)
    now = time.time()
    expires = now + PAYMENT_ORDER_TTL_SECONDS
    code = _payment_order_code()
    async with _db_lock:
        def work():
            conn = _db_connect()
            try:
                ...
                if own:
                    ...
                    out = dict(own)
                    conn.close()
                    return {"ok": False, "open_pending": True, "existing": out}
                existing = conn.execute(
                    "SELECT * FROM payment_orders WHERE status='pending' AND payer_phone=? AND ABS(amount-?) < 0.01 AND expires_at>? ORDER BY created_at LIMIT 1",
                    (payer_phone, float(amount), now),
                ).fetchone()
                if existing and int(existing["user_id"]) != int(user_id):
                    conn.execute("COMMIT")
                    out = dict(existing)
                    conn.close()
                    return {"ok": False, "conflict": True, "existing": out}
                conn.execute(
                    """INSERT INTO payment_orders(order_code,user_id,plan,days,plan_title,amount,payer_phone,status,created_at,expires_at,note)
                       VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
                    (code, int(user_id), plan, int(days), plan_title, float(amount), payer_phone, "pending", now, expires, "created from bot"),
                )
                row = conn.execute("SELECT * FROM payment_orders WHERE order_code=?", (code,)).fetchone()
                conn.execute("COMMIT")
                out = dict(row)
                conn.close()
...
async def db_get_payment_order(order_id: int) -> Optional[dict]:
    def work():
        conn = _db_connect()
        row = conn.execute("SELECT * FROM payment_orders WHERE id=?", (int(order_id),)).fetchone()
        conn.close()
        return dict(row) if row else None
    return await asyncio.to_thread(work)


def _base_sms_fingerprint(sender: str, sms_text: str) -> str:
    sender_norm = normalize_sms_sender(sender or "Almadar")
    compact_text = re.sub(r"\s+", " ", str(sms_text or "").strip())
    return hashlib.sha256((sender_norm + "\n" + compact_text).encode("utf-8", errors="ignore")).hexdigest()


async def db_record_incoming_madar_sms(sender: str, sms_text: str, phone: str = "", amount: Optional[float] = None, status: str = "unclaimed", note: str = "") -> dict:
    """Store every allowed Almadar SMS in the safety ledger. Duplicate blocking lasts SMS_DUPLICATE_TTL_DAYS."""
    now = time.time()
    phone = normalize_madar_phone(phone)
    amount_value = float(amount) if amount is not None else None
    status = status if status in {"unclaimed", "claimed", "duplicate", "ignored"} else "unclaimed"
    base_hash = _base_sms_fingerprint(sender, sms_text)
    cutoff_dup = now - (float(SMS_DUPLICATE_TTL_DAYS) * 86400)
    cutoff_keep = now - (int(PAYMENT_SMS_RETENTION_DAYS) * 86400)
    async with _db_lock:
        def work():
            conn = _db_connect()
            try:
                conn.execute("BEGIN IMMEDIATE")
                conn.execute("DELETE FROM unclaimed_madar_sms WHERE received_at < ? AND status IN ('unclaimed','duplicate','ignored')", (cutoff_keep,))
                previous = conn.execute(
                    "SELECT * FROM unclaimed_madar_sms WHERE sender=? AND raw_text=? AND received_at>=? ORDER BY received_at DESC LIMIT 1",
                    (str(sender or ""), str(sms_text or ""), cutoff_dup),
                ).fetchone()
                final_status = status
                final_note = note or ""
                is_duplicate = False
                if previous:
                    ...
                    final_note = (final_note + " | " if final_note else "") + f"duplicate of ledger #{previous['id']} within {SMS_DUPLICATE_TTL_DAYS:g} days"
                    is_duplicate = True
                    sms_hash = hashlib.sha256((base_hash + ":duplicate:" + str(now) + ":" + uuid.uuid4().hex).encode()).hexdigest()
                else:
                    sms_hash = base_hash
                try:
                    conn.execute(
                        """INSERT INTO unclaimed_madar_sms(sms_hash,sender,raw_text,parsed_phone,parsed_amount,received_at,status,claimed_order_id,claimed_user_id,claimed_at,note)
                           VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
                        (sms_hash, str(sender or ""), str(sms_text or "")[:8000], phone or "", amount_value, now, final_status, 0, 0, 0, final_note),
                    )
                except sqlite3.IntegrityError:
                    sms_hash = hashlib.sha256((base_hash + ":retry:" + str(now) + ":" + uuid.uuid4().hex).encode()).hexdigest()
                    final_status = "duplicate"
                    is_duplicate = True
                    conn.execute(
                        """INSERT INTO unclaimed_madar_sms(sms_hash,sender,raw_text,parsed_phone,parsed_amount,received_at,status,claimed_order_id,claimed_user_id,claimed_at,note)
                           VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
                        (sms_hash, str(sender or ""), str(sms_text or "")[:8000], phone or "", amount_value, now, final_status, 0, 0, 0, final_note or "duplicate hash retry"),
                    )
                row = conn.execute("SELECT * FROM unclaimed_madar_sms WHERE sms_hash=?", (sms_hash,)).fetchone()
                conn.execute("COMMIT")
                out = dict(row)
                conn.close()
                return {"ok": True, "new": not is_duplicate, "duplicate": is_duplicate, "sms_id": out["id"], "row": out}
            except Exception:
                try: conn.execute("ROLLBACK")
                except Exception: pass
                conn.close()
...
async def db_record_processed_sms(sms_text: str, phone: str = "", amount: Optional[float] = None, result: str = "received") -> bool:
    """Legacy duplicate table kept for compatibility. New safety logic uses unclaimed_madar_sms."""
    sms_hash = hashlib.sha256((sms_text or "").encode("utf-8", errors="ignore")).hexdigest()
    cutoff = time.time() - (float(SMS_DUPLICATE_TTL_DAYS) * 86400)
    async with _db_lock:
        def work():
            conn = _db_connect()
            conn.execute("DELETE FROM processed_sms WHERE COALESCE(processed_at, 0) < ?", (cutoff,))
            old = conn.execute("SELECT sms_hash FROM processed_sms WHERE sms_hash=?", (sms_hash,)).fetchone()
            if old:
                conn.commit(); conn.close(); return False
            conn.execute(
                "INSERT INTO processed_sms(sms_hash,sms_text,parsed_phone,parsed_amount,processed_at,result) VALUES(?,?,?,?,?,?)",
                (sms_hash, sms_text, phone or "", amount if amount is not None else None, time.time(), result),
            )
            conn.commit(); conn.close(); return True
        return await asyncio.to_thread(work)


async def db_find_matching_payment_order(phone: str, amount: float) -> dict:
    phone = normalize_madar_phone(phone)
    now = time.time()
    def work():
        conn = _db_connect()
        conn.execute("UPDATE payment_orders SET status='expired', note='auto expired' WHERE status='pending' AND expires_at < ?", (now,))
        rows = conn.execute(
            "SELECT * FROM payment_orders WHERE status='pending' AND payer_phone=? AND ABS(amount-?) < 0.01 AND expires_at>? ORDER BY created_at",
            (phone, float(amount), now),
        ).fetchall()
        conn.commit(); conn.close()
        return [dict(r) for r in rows]
    rows = await asyncio.to_thread(work)
    if len(rows) == 1:
        return {"status": "match", "order": rows[0]}
    if len(rows) > 1:
        return {"status": "ambiguous", "orders": rows}
    return {"status": "none", "orders": []}


async def db_mark_payment_order_status(order_id: int, status: str, matched_sms: str = "", note: str = "") -> None:
    async with _db_lock:
        def work():
            conn = _db_connect()
            conn.execute(
                "UPDATE payment_orders SET status=?, matched_sms=COALESCE(NULLIF(?, ''), matched_sms), matched_at=?, note=COALESCE(NULLIF(?, ''), note) WHERE id=?",
                (status, matched_sms or "", time.time(), note or "", int(order_id)),
            )
            conn.commit(); conn.close()
        await asyncio.to_thread(work)


async def db_find_unclaimed_madar_sms(phone: str, amount: float, within_seconds: int = PAYMENT_UNCLAIMED_MATCH_SECONDS) -> Optional[dict]:
    phone = normalize_madar_phone(phone)
    cutoff = time.time() - int(within_seconds)
    def work():
        conn = _db_connect()
        row = conn.execute(
            """SELECT * FROM unclaimed_madar_sms
               WHERE status='unclaimed' AND parsed_phone=? AND ABS(parsed_amount-?) < 0.01 AND received_at>=?
               ORDER BY received_at DESC LIMIT 1""",
            (phone, float(amount), cutoff),
        ).fetchone()
        conn.close()
        return dict(row) if row else None
    return await asyncio.to_thread(work)
...
async def db_find_partial_sms_for_order(order: dict, within_seconds: int = PAYMENT_UNCLAIMED_MATCH_SECONDS, limit: int = 3) -> list:
    if not order:
        return []
    cutoff = time.time() - int(within_seconds)
    phone = normalize_madar_phone(order.get("payer_phone", ""))
    amount = float(order.get("amount") or 0)
    def work():
        conn = _db_connect()
        rows = conn.execute(
            """SELECT * FROM unclaimed_madar_sms
               WHERE status='unclaimed' AND received_at>=? AND (parsed_phone=? OR ABS(parsed_amount-?) < 0.01)
               ORDER BY received_at DESC LIMIT ?""",
            (cutoff, phone, amount, int(limit)),
        ).fetchall()
        conn.close()
        return [dict(r) for r in rows]
    return await asyncio.to_thread(work)
...
async def db_atomic_approve_order_with_sms(order_id: int, sms_id: int, note: str = "atomic auto approval") -> dict:
    now = time.time()
    async with _db_lock:
        def work():
            conn = _db_connect()
            try:
                conn.execute("BEGIN IMMEDIATE")
                order = conn.execute("SELECT * FROM payment_orders WHERE id=?", (int(order_id),)).fetchone()
                sms = conn.execute("SELECT * FROM unclaimed_madar_sms WHERE id=?", (int(sms_id),)).fetchone()
                if not order:
                    conn.execute("ROLLBACK"); conn.close(); return {"ok": False, "status": "order_missing"}
                if not sms:
                    conn.execute("ROLLBACK"); conn.close(); return {"ok": False, "status": "sms_missing"}
                order = dict(order); sms = dict(sms)
                if order.get("status") != "pending":
                    conn.execute("ROLLBACK"); conn.close(); return {"ok": False, "status": "order_not_pending", "order": order}
                if float(order.get("expires_at") or 0) <= now:
                    conn.execute("UPDATE payment_orders SET status='expired', note='auto expired before claim' WHERE id=?", (int(order_id),))
                    conn.execute("COMMIT"); conn.close(); return {"ok": False, "status": "order_expired", "order": order}
                if sms.get("status") != "unclaimed" or int(sms.get("claimed_order_id") or 0):
                    conn.execute("ROLLBACK"); conn.close(); return {"ok": False, "status": "sms_not_available", "sms": sms}
                if normalize_madar_phone(sms.get("parsed_phone", "")) != normalize_madar_phone(order.get("payer_phone", "")):
                    conn.execute("ROLLBACK"); conn.close(); return {"ok": False, "status": "phone_mismatch", "order": order, "sms": sms}
                if sms.get("parsed_amount") is None or abs(float(sms.get("parsed_amount") or 0) - float(order.get("amount") or 0)) >= 0.01:
                    conn.execute("ROLLBACK"); conn.close(); return {"ok": False, "status": "amount_mismatch", "order": order, "sms": sms}
                if float(sms.get("received_at") or 0) < now - int(PAYMENT_UNCLAIMED_MATCH_SECONDS):
                    conn.execute("ROLLBACK"); conn.close(); return {"ok": False, "status": "sms_too_old", "order": order, "sms": sms}
                _set_user_plan_in_conn(conn, int(order["user_id"]), order["plan"], int(order["days"]))
                conn.execute(
                    "UPDATE payment_orders SET status='auto_approved', matched_sms=?, matched_at=?, note=? WHERE id=? AND status='pending'",
                    (str(sms.get("raw_text") or "")[:4000], now, note, int(order_id)),
                )
                conn.execute(
                    """UPDATE unclaimed_madar_sms
                       SET status='claimed', claimed_order_id=?, claimed_user_id=?, claimed_at=?, note=?
                       WHERE id=? AND status='unclaimed'""",
                    (int(order_id), int(order["user_id"]), now, note, int(sms_id)),
                )
                conn.execute("COMMIT")
                conn.close()
                order["matched_sms"] = sms.get("raw_text", "")
                return {"ok": True, "status": "claimed", "order": order, "sms": sms}
...
async def db_atomic_approve_matching_sms(phone: str, amount: float, sms_id: int, note: str = "atomic auto approval by Almadar SMS") -> dict:
    phone = normalize_madar_phone(phone)
    now = time.time()
    def find_candidates():
        conn = _db_connect()
        ...
        conn.commit(); conn.close()
        return [dict(r) for r in rows]
    rows = await asyncio.to_thread(find_candidates)
    if len(rows) == 1:
        return await db_atomic_approve_order_with_sms(int(rows[0]["id"]), int(sms_id), note=note)
    if len(rows) > 1:
        return {"ok": False, "status": "ambiguous", "orders": rows}
    return {"ok": False, "status": "none", "orders": []}


async def try_auto_approve_order_from_stored_sms(order: dict) -> bool:
    if not order:
        return False
    sms_row = await db_find_unclaimed_madar_sms(order.get("payer_phone", ""), float(order.get("amount") or 0))
    if not sms_row:
        return False
    result = await db_atomic_approve_order_with_sms(int(order["id"]), int(sms_row["id"]), note="auto approved from previous unclaimed Almadar SMS")
    return bool(result.get("ok"))


async def db_manual_approve_payment_order(order_id: int, admin_id: int = 0) -> dict:
    now = time.time()
    async with _db_lock:
        def work():
            conn = _db_connect()
            try:
                ...
                row = conn.execute("SELECT * FROM payment_orders WHERE id=?", (int(order_id),)).fetchone()
                if not row:
                    conn.execute("ROLLBACK"); conn.close(); return {"ok": False, "status": "missing"}
                order = dict(row)
                if order.get("status") != "pending":
                    conn.execute("ROLLBACK"); conn.close(); return {"ok": False, "status": "not_pending", "order": order}
                _set_user_plan_in_conn(conn, int(order["user_id"]), order["plan"], int(order["days"]))
                conn.execute(
                    "UPDATE payment_orders SET status='manual_approved', matched_at=?, note=? WHERE id=? AND status='pending'",
                    (now, f"approved by admin {admin_id}", int(order_id)),
                )
                conn.execute("COMMIT"); conn.close()
                return {"ok": True, "status": "manual_approved", "order": order}
            except Exception as exc:
                try: conn.execute("ROLLBACK")
                except Exception: pass
                conn.close(); return {"ok": False, "status": "exception", "error": str(exc)}
        return await asyncio.to_thread(work)


async def db_list_recent_madar_sms(limit: int = 20) -> list:
    def work():
        conn = _db_connect()
        rows = conn.execute(
            "SELECT * FROM unclaimed_madar_sms ORDER BY received_at DESC LIMIT ?",
            (int(limit),),
        ).fetchall()
        conn.close()
        return [dict(r) for r in rows]
...
# Backward-compatible wrappers for old unmatched_madar_sms API. New safety ledger is the source of truth.
async def db_store_unmatched_madar_sms(phone: str, amount: float, sms_text: str) -> None:
    await db_record_incoming_madar_sms("Almadar", sms_text, phone, amount, status="unclaimed", note="legacy store wrapper")

async def db_find_unmatched_madar_sms(phone: str, amount: float) -> Optional[dict]:
    return await db_find_unclaimed_madar_sms(phone, amount)

async def db_mark_unmatched_madar_sms_used(row_id: int, order_id: int) -> None:
    async with _db_lock:
        def work():
            conn = _db_connect()
            conn.execute(
                "UPDATE unclaimed_madar_sms SET status='claimed', claimed_order_id=?, claimed_at=? WHERE id=? AND status='unclaimed'",
                (int(order_id), time.time(), int(row_id)),
            )
            conn.commit(); conn.close()
        await asyncio.to_thread(work)
...
def escape_md(text: object) -> str:
    ...
    for ch in ("*", "_", "`", "["): value = value.replace(ch, "\\" + ch)
    return value
...
SUMMARY_PROMPTS = {
    "disease_v2": f"""{ACADEMIC_PREFIX}
اكتب ملخصاً طبياً عربياً منظماً ومناسباً لطلاب الطب. لا تكتب نصاً متلاصقاً.
التزم بهذا الشكل قدر الإمكان:
📚 ملخص المحاضرة
━━━━━━━━━━━━━━
🧠 الموضوع: استنتج عنواناً قصيراً من المحاضرة
🎯 الهدف: فهم النقاط الامتحانية المهمة
...
3️⃣ النقاط الامتحانية الذهبية
⭐ ...

4️⃣ الجداول والمقارنات
- اكتب الجداول كنقاط منظمة وواضحة.

5️⃣ التشخيص / Management إن وجد
🩺 Diagnosis:
• ...
💊 Management:
• ...

6️⃣ أخطاء شائعة في الامتحان
⚠️ ...

7️⃣ خلاصة سريعة للحفظ
✅ ...
...
2. ...
3. ...
استخدم لغة عربية واضحة، واحفظ المصطلحات الطبية الإنجليزية المهمة بين قوسين. لا تستخدم Markdown معقد.""",
    "highyield": f"""{ACADEMIC_PREFIX}
اكتب High-yield notes بشكل منظم للامتحان. استخدم عناوين مرقمة، نقاط قصيرة، Pearls، أخطاء شائعة، وأسئلة مراجعة سريعة. لا تستخدم Markdown معقد.""",
    "osce": f"""{ACADEMIC_PREFIX}
اكتب ملخص OSCE منظم: History, Examination, Investigations, Diagnosis, Management, Counseling, Red flags. استخدم نقاط واضحة وإيموجي طبي بسيط دون Markdown معقد.""",
}
...
def format_summary_for_student(summary: str, style: str = "disease_v2") -> str:
    body = clean_telegram_text(summary)
    if not body:
        return ""
    has_good_header = "ملخص المحاضرة" in body or "الفكرة العامة" in body or "النقاط الامتحانية" in body
    if has_good_header:
        return body
    return (
        "📚 ملخص المحاضرة\n"
        "━━━━━━━━━━━━━━\n"
        "🧠 الموضوع: من المحاضرة المرسلة\n"
        "🎯 الهدف: فهم النقاط الامتحانية المهمة\n\n"
        "1️⃣ الفكرة العامة\n"
        f"{body}\n\n"
        "2️⃣ النقاط الامتحانية الذهبية\n"
        "⭐ راجع التعريفات والأرقام والخطوات العلاجية المذكورة أعلاه.\n\n"
        "3️⃣ خلاصة سريعة للحفظ\n"
        "✅ ركّز على التشخيص، التفريقات، وخطوات management.\n"
    )
...
def split_telegram_text(text: str, limit: int = 3800) -> List[str]:
    ...
    if len(chunks) > 1:
        total = len(chunks)
        chunks = [f"📚 ملخص المحاضرة — الجزء {i+1}/{total}\n━━━━━━━━━━━━━━\n\n{chunk}" for i, chunk in enumerate(chunks)]
    return chunks
    ...
    if isinstance(options, dict):
        for letter in letters:
            ...
            if isinstance(val, dict):
                val = val.get("text") or val.get("option") or val.get("value") or val.get("answer") or val.get("label") or ""
...
async def get_or_create_summary(file_hash: str, text: str, style: str) -> str:
    ...
    async with lock:
        ...
        if cached:
            return format_summary_for_student(cached, style)
        summary = ""
        try:
            prompt = (
                f"{SUMMARY_PROMPTS.get(style, SUMMARY_PROMPTS['disease_v2'])}\n\n"
                "SOURCE LECTURE TEXT:\n"
                f"{representative_text(text, MAX_TEXT_CHARS_FOR_AI)}"
            )
            summary = await call_gemini(prompt, temperature=0.2, max_output_tokens=8192)
        except Exception:
            logger.exception("summary failed")
        formatted = format_summary_for_student(summary, style)
        if formatted and formatted.strip():
            await db_save_summary(file_hash, style, formatted)
        return formatted
...
async def stage_start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["active_staging"] = {"uni": _pp_canonical_uni(context.args[0]), "subject": _pp_canonical_subject(context.args[1])}
...
async def upload_final_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        return await update.message.reply_text("استخدم: /upload_final [uni] [subject] ثم أرسل ملف JSON الجاهز")
    uni = _pp_canonical_uni(context.args[0])
    subject = _pp_canonical_subject(context.args[1])
    context.user_data.pop("active_staging", None)
    context.user_data["pending_final"] = {"uni": uni, "subject": subject}
    await update.message.reply_text(
        "📤 وضع رفع ملف JSON جاهز\n\n"
        f"القسم: {uni} / {subject}\n\n"
        "أرسل الآن ملف JSON فقط.\n"
        "سأحفظ الأسئلة مباشرة في بنك السنوات السابقة الذي يستخدمه تدريب الطلبة."
    )
...
def mode_keyboard(user_data: dict) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton("🎯 ابدأ تدريب", callback_data="mode:quiz"), InlineKeyboardButton("📚 بنك السنوات السابقة", callback_data="pp:home")],
        [InlineKeyboardButton("📄 لخص محاضرة", callback_data="mode:summary"), InlineKeyboardButton("📝 أسئلة من محاضرتي", callback_data="mode:quiz")],
        [InlineKeyboardButton("🧪 OSCE", callback_data="mode:osce"), InlineKeyboardButton("🧠 ماذا أدرس الآن؟", callback_data="panel:study_now")],
        [InlineKeyboardButton("📊 تقدمي", callback_data="panel:progress"), InlineKeyboardButton("💎 اشتراكي", callback_data="panel:subscription")],
        [InlineKeyboardButton("☎️ الدعم", callback_data="panel:support")],
    ]
...
def main_menu_text(user_data: dict) -> str:
    plan = user_data.get("plan_label", "Free")
    expires = user_data.get("plan_expires_text", "لا يوجد")
    used = int(user_data.get("past_used_today", 0) or 0)
    uni = user_data.get("current_uni_label") or "غير محددة"
    subject = user_data.get("current_subject_label") or "غير محددة"
    return (
        "🩺 MedMCQ\n"
        "━━━━━━━━━━━━━━\n"
        "لوحة تدريبك الطبية الذكية\n\n"
        f"👤 خطتك: {plan}\n"
        f"⏳ الاشتراك: {expires}\n"
        f"🔥 تدريبك اليوم: {used}/{PAST_PAPER_FREE_DAILY_LIMIT} من بنك السنوات السابقة\n"
        f"📚 الجامعة الحالية: {uni}\n"
...
async def send_current_question(chat_id: int, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if idx >= len(questions):
        ...
        return
    if not await pp_before_send_question_if_needed(chat_id, context):
        return
    await safe_send(
        context.bot,
        chat_id,
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["is_admin"] = is_admin_user(uid)
    await prepare_dashboard_data(uid, context.user_data)
    await update.message.reply_text(main_menu_text(context.user_data), reply_markup=mode_keyboard(context.user_data))
...
async def support_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        "☎️ الدعم\n\n"
        "اختر ما تريد:\n\n"
        "• تواصل مع الدعم: لإرسال إثبات الدفع أو الاستفسار.\n"
        "• الإبلاغ عن مشكلة: لو واجهت خطأ في البوت."
    )
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("☎️ تواصل مع الدعم", callback_data="support:contact")],
        [InlineKeyboardButton("🚨 الإبلاغ عن مشكلة", callback_data="support:problem")],
        [InlineKeyboardButton("🔙 رجوع", callback_data="main:home")],
    ])
...
def _pp_canonical_uni(value: object) -> str:
    """Canonical university key used in storage and retrieval."""
    raw = str(value or "").strip().lower()
    raw = re.sub(r"[\s_\-]+", "", raw)
    aliases = {
        "ajdabiya": "ajdabiya", "اجدابيا": "ajdabiya", "إجدابيا": "ajdabiya", "اجدايبا": "ajdabiya",
        "benghazi": "benghazi", "بنغازي": "benghazi", "banghazi": "benghazi",
    }
    return aliases.get(raw, raw or "unknown")


def _pp_canonical_subject(value: object) -> str:
    """Canonical subject key. obgyn/Ob-Gyn/Ob_Gyn/Ob/Gyn are treated as the same."""
    raw = str(value or "").strip().lower()
    compact = re.sub(r"[\s_\-/\\]+", "", raw)
    aliases = {
        "obgyn": "obgyn", "obgyne": "obgyn", "obstetricsgynecology": "obgyn", "obsandgyn": "obgyn",
        "obstetrics": "obgyn", "gynecology": "obgyn", "gynaecology": "obgyn", "نساءوولادة": "obgyn", "نساء": "obgyn",
        "forensic": "forensic", "طبشرعي": "forensic",
        "ophthalmology": "ophthalmology", "عيون": "ophthalmology",
        "radiology": "radiology", "اشعة": "radiology", "أشعة": "radiology",
        "community": "community", "مجتمع": "community", "communitymedicine": "community",
    }
    return aliases.get(compact, compact or "unknown")


def _pp_option_letters() -> List[str]:
    return list("ABCDE")


def _pp_extract_json_question_list(obj: object) -> List[dict]:
    """Accept JSON shapes: list, {questions/items/data/mcqs: [...]} and never invent data."""
    if isinstance(obj, list):
        return obj
    if isinstance(obj, dict):
        for key in ("questions", "items", "data", "mcqs"):
            val = obj.get(key)
...
def _pp_answer_to_letter(answer: object, options: Dict[str, str]) -> str:
    """Convert explicit answer value to A-E. Empty/unknown answer stays invalid."""
    raw = str(answer or "").strip()
    if not raw:
        return ""
    cleaned = raw.strip().upper()
    cleaned = re.sub(r"^(ANSWER|CORRECT|OPTION|CHOICE|KEY)\s*[:=\-]?\s*", "", cleaned, flags=re.I).strip()
    cleaned = cleaned.strip(" .:;-)('[]{}")
    if cleaned in _pp_option_letters():
        return cleaned
    m = re.match(r"^([A-E])\b", cleaned)
    if m:
        return m.group(1).upper()
    # Numeric answers like 1/2/3/4/5
    if cleaned.isdigit():
        idx = int(cleaned) - 1
        letters = _pp_option_letters()
        if 0 <= idx < len(letters):
            return letters[idx]
    # Match answer text exactly after normalization.
    ans_clean = _pp_clean_text(raw)
    for letter, val in (options or {}).items():
        if ans_clean and ans_clean == _pp_clean_text(val):
            return str(letter).upper()
    return ""


def _pp_options_from_any(options: object) -> Dict[str, str]:
    """Normalize options/choices/answers into A-E without making up choices."""
    letters = _pp_option_letters()
    out: Dict[str, str] = {}
    if isinstance(options, dict):
        # Direct A/B/C keys.
        for letter in letters:
            ...
            if val is not None and str(val).strip():
                out[letter] = str(val).strip()
        # Dict of arbitrary keys -> values, keep insertion order if A-E keys were not found.
        if not out:
            for idx, (_k, val) in enumerate(options.items()):
                if idx >= len(letters):
                    break
                if isinstance(val, dict):
                    val = val.get("text") or val.get("option") or val.get("value") or val.get("answer") or val.get("label") or ""
        ...
        for idx, item in enumerate(options[:5]):
            ...
            val = item
            if isinstance(item, dict):
                val = item.get("text") or item.get("option") or item.get("value") or item.get("answer") or item.get("label") or ""
            if val is not None and str(val).strip():
                out[letters[idx]] = str(val).strip()
    return out


def _pp_answer_from_marked_option(options: object) -> str:
    """Accept explicit flags inside choices, e.g. {text:'...', correct:true}."""
    letters = _pp_option_letters()
    if isinstance(options, list):
        for idx, item in enumerate(options[:5]):
            if isinstance(item, dict) and bool(item.get("correct") or item.get("is_correct") or item.get("right")):
                return letters[idx]
    if isinstance(options, dict):
        for idx, (key, item) in enumerate(options.items()):
            if isinstance(item, dict) and bool(item.get("correct") or item.get("is_correct") or item.get("right")):
...
def pp_subject_menu_keyboard(uni: str, subject: str, has_wrong: bool = False) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton("⚡ اختبار سريع 20 سؤال", callback_data=f"pp:quiz:{uni}:{subject}")],
        ...
        [InlineKeyboardButton("❌ أسئلتي الخاطئة", callback_data=f"pp:wrong:{uni}:{subject}")],
        [InlineKeyboardButton("📊 تحليلي", callback_data=f"pp:progress:{uni}:{subject}")],
        [InlineKeyboardButton("⬅️ رجوع", callback_data=f"pp:uni:{uni}")],
    ]
    return InlineKeyboardMarkup(rows)
...
def _pp_question_fingerprint(q: dict) -> str:
    """Stable duplicate key using question + options + answer, as required for ready JSON uploads."""
    if not isinstance(q, dict):
        return ""
    question = _pp_clean_text(q.get("question", ""))
    options = q.get("options", {}) if isinstance(q.get("options", {}), dict) else {}
    opt_blob = "|".join(f"{l}:{_pp_clean_text(options.get(l, ''))}" for l in _pp_option_letters())
    answer = str(q.get("correct", "")).strip().upper()
    base = f"{question}||{opt_blob}||{answer}"
    return hashlib.sha256(base.encode("utf-8")).hexdigest()

def _pp_normalize_question(q: dict) -> Optional[dict]:
    """Normalize a ready MCQ. If no explicit answer exists, reject it; never invent answers."""
    if not isinstance(q, dict):
        return None
    ...
    if raw_options is None:
        raw_options = q.get("answers")
    options = _pp_options_from_any(raw_options)
    ...
    answer = _pp_answer_to_letter(answer_value, options)
    if not answer:
        answer = _pp_answer_from_marked_option(raw_options)

    explanation = str(q.get("explanation") or q.get("rationale") or q.get("explain") or q.get("note") or "").strip()

    if not question or not options:
        return None
    # Require at least A-D. Do not fabricate options.
    for letter in "ABCD":
        if not options.get(letter):
            return None
    # E is optional; keep it only if present.
    norm_opts = {letter: str(options.get(letter, "")).strip() for letter in _pp_option_letters() if str(options.get(letter, "")).strip()}

    if answer not in norm_opts:
        return None
    ...
    return {
        ...
        "options": norm_opts,
        "correct": answer,
        "explanation": explanation or "من أسئلة السنوات السابقة.",
    }

async def pp_load_questions(uni: str, subject: str) -> List[dict]:
    uni_key = _pp_canonical_uni(uni)
    subject_key = _pp_canonical_subject(subject)

    def work():
        conn = _db_connect()
        rows = conn.execute("SELECT uni, subject, questions_json FROM past_papers ORDER BY id ASC").fetchall()
        conn.close()
        out = []
        seen = set()
        for row in rows:
            if _pp_canonical_uni(row["uni"]) != uni_key or _pp_canonical_subject(row["subject"]) != subject_key:
                continue
            try:
...
async def pp_filter_new_questions(uni: str, subject: str, incoming: list) -> Tuple[List[dict], int, int]:
    """Return (new_questions, duplicate_count, invalid_count). Reject missing answers."""
    if not isinstance(incoming, list):
        return [], 0, 1

    uni_key = _pp_canonical_uni(uni)
    subject_key = _pp_canonical_subject(subject)
    existing_questions = await pp_load_questions(uni_key, subject_key)
...
async def pp_admin_stats_text() -> str:
    def work():
        ...
        for row in rows:
            key = (_pp_canonical_uni(row["uni"]), _pp_canonical_subject(row["subject"]))
...
async def pp_is_unlimited_user(user_id: int) -> bool:
    if is_admin_user(user_id):
        return True
    plan = await db_get_user_plan(user_id)
    return plan in {"premium", "premium_plus", "admin"}

async def pp_free_remaining(user_id: int) -> int:
    if await pp_is_unlimited_user(user_id):
        return 10**9
    used = await db_get_daily_usage(user_id, PAST_PAPER_USAGE_KIND)
    return max(0, PAST_PAPER_FREE_DAILY_LIMIT - used)

def pp_limit_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("💎 اشترك الآن", callback_data="panel:subscription")],
        [InlineKeyboardButton("⬅️ الرئيسية", callback_data="main:home")],
    ])

def pp_limit_message() -> str:
    return (
        "🔒 وصلت للحد المجاني اليومي\n"
        f"استخدمت {PAST_PAPER_FREE_DAILY_LIMIT} سؤالًا من بنك السنوات السابقة اليوم.\n\n"
        "💎 اشترك Premium وافتح:\n"
        "• كل بنك السنوات السابقة بدون حد\n"
        "• تدريب عشوائي كما تريد\n"
        "• حل بالترتيب\n"
        "• مراجعة الأخطاء\n"
        "• تحليلك وتقدمك"
    )

async def pp_guard_before_start(update: Update, context: ContextTypes.DEFAULT_TYPE, requested: int = 1) -> Tuple[bool, int]:
    query = update.callback_query
    user_id = query.from_user.id if query and query.from_user else update.effective_chat.id
    if await pp_is_unlimited_user(user_id):
        return True, requested
    remaining = await pp_free_remaining(user_id)
    if remaining <= 0:
        await safe_edit(query, pp_limit_message(), reply_markup=pp_limit_keyboard())
        return False, 0
    return True, min(int(requested), remaining)

async def pp_before_send_question_if_needed(chat_id: int, context: ContextTypes.DEFAULT_TYPE) -> bool:
    session = context.user_data.get("session")
    if not isinstance(session, dict):
        return True
    level = str(session.get("level", ""))
    if not level.startswith("past:"):
        return True
    user_id = int(chat_id)
    if await pp_is_unlimited_user(user_id):
        return True
    remaining = await pp_free_remaining(user_id)
    if remaining <= 0:
        await context.bot.send_message(chat_id=chat_id, text=pp_limit_message(), reply_markup=pp_limit_keyboard())
        context.user_data.pop("session", None)
        return False
    await db_increment_daily_usage(user_id, PAST_PAPER_USAGE_KIND, 1)
    return True

async def pp_get_weakest_section(user_id: int) -> Optional[dict]:
    def work():
        conn = _db_connect()
        rows = conn.execute("SELECT * FROM past_progress WHERE user_id=? AND attempts > 0", (int(user_id),)).fetchall()
        conn.close()
        best = None
        for r in rows:
            d = dict(r)
            ...
            score = (acc, -wrong, -attempts)
            item = {"uni": d.get("uni"), "subject": d.get("subject"), "attempts": attempts, "correct": correct, "wrong": wrong, "accuracy": round(acc)}
...
async def prepare_dashboard_data(user_id: int, user_data: dict) -> None:
    info = await db_get_user_plan_info(user_id)
    plan = info.get("plan", "free")
    exp = float(info.get("plan_expires_at") or 0)
    user_data["plan_label"] = PLAN_LABELS.get(plan, plan)
    user_data["plan_expires_text"] = "لا يوجد" if not exp else datetime.fromtimestamp(exp).strftime("%Y-%m-%d")
    user_data["past_used_today"] = await db_get_daily_usage(user_id, PAST_PAPER_USAGE_KIND)
    last_uni = user_data.get("current_uni") or user_data.get("last_uni") or ""
    last_subject = user_data.get("current_subject") or user_data.get("last_subject") or ""
    user_data["current_uni_label"] = UNI_LABELS.get(_pp_canonical_uni(last_uni), "غير محددة") if last_uni else "غير محددة"
    user_data["current_subject_label"] = SUBJECT_LABELS.get(_pp_canonical_subject(last_subject), "غير محددة") if last_subject else "غير محددة"
...
async def pp_home_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━\n\n"
        "اختر الجامعة.\n\n"
        f"Free: تجربة حتى {PAST_PAPER_FREE_DAILY_LIMIT} سؤال يوميًا.\n"
        "Premium: البنك كامل بدون حد."
    )
    await safe_edit(query, text, reply_markup=past_home_keyboard())
...
async def pp_uni_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = _safe_callback_data(query.data or "")
    parts = data.split(":")
    uni = _pp_canonical_uni(parts[2] if len(parts) >= 3 else "ajdabiya")
...
async def pp_sub_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) < 4:
        return await pp_home_callback(update, context)
    uni, subject = _pp_canonical_uni(parts[2]), _pp_canonical_subject(parts[3])
    context.user_data["current_uni"] = uni
    ...
    uni_label = UNI_LABELS.get(uni, uni)
    subject_label = SUBJECT_LABELS.get(subject, subject)
    suggestion = "ابدأ باختبار سريع 20 سؤال حتى أحدد مستواك." if attempts == 0 else ("راجع أخطاءك ثم حل اختبار سريع." if pct < 80 else "ممتاز، جرّب حل بالترتيب أو انتقل لقسم آخر.")
    text = (
        f"🎯 تدريب {subject_label} — {uni_label}\n"
        "━━━━━━━━━━━━━━\n\n"
        f"📦 الأسئلة المتاحة: {len(questions)}\n"
        f"📝 محاولاتك: {attempts}\n"
        f"✅ الصحيحة: {correct}\n"
        f"❌ الخاطئة: {wrong}\n"
        f"📈 دقتك: {pct}%\n\n"
...
async def pp_start_session(update: Update, context: ContextTypes.DEFAULT_TYPE, uni: str, subject: str, mode: str):
    query = update.callback_query
    uni = _pp_canonical_uni(uni)
    subject = _pp_canonical_subject(subject)
    context.user_data["current_uni"] = uni
    context.user_data["current_subject"] = subject
    questions = await pp_load_questions(uni, subject)
    if not questions:
    ...
    requested = 1 if mode == "random" else (min(20, len(questions)) if mode == "quiz" else len(questions))
    ok, allowed_count = await pp_guard_before_start(update, context, requested)
    if not ok:
        return
    if mode == "random":
        selected = [random.choice(questions)]
    elif mode == "seq":
        selected = questions[:allowed_count]
    elif mode == "quiz":
        selected = random.sample(questions, min(allowed_count, 20, len(questions)))
    else:
        selected = questions[:allowed_count]
    if not selected:
        return await safe_edit(query, pp_limit_message(), reply_markup=pp_limit_keyboard())
    ...
    note = ""
    user_id = query.from_user.id if query and query.from_user else update.effective_chat.id
    if not await pp_is_unlimited_user(user_id):
        remaining_after = max(0, await pp_free_remaining(user_id) - len(selected))
        note = f"\n\nFree: سيتم احتساب كل سؤال عند عرضه. المتبقي بعد هذه الجلسة تقريبًا: {remaining_after}"
    await safe_edit(query, f"✅ بدأ التدريب\n🏛️ {UNI_LABELS.get(uni, uni)}\n📚 {SUBJECT_LABELS.get(subject, subject)}\n📝 عدد الأسئلة: {len(selected)}{note}")
...
async def pp_wrong_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ok, allowed_count = await pp_guard_before_start(update, context, len(wrong_list))
    if not ok:
        return
    wrong_list = wrong_list[:allowed_count]
    context.user_data["session"] = {
        "quiz_id": str(uuid.uuid4())[:8],
...
async def pp_progress_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    wrong = int(prog.get("wrong") or 0)
    pct = round(correct / attempts * 100) if attempts else 0
    advice = "ابدأ باختبار سريع حتى يظهر تحليل أدق." if attempts == 0 else ("راجع الأخطاء أولاً؛ هذا أسرع طريق لرفع درجتك." if pct < 75 else "مستواك جيد، استمر بحل بالترتيب لتغطية البنك.")
    msg = (
        "📊 تحليلي\n"
        "━━━━━━━━━━━━━━\n\n"
        f"🏛️ {UNI_LABELS.get(uni, uni)}\n"
        f"📚 {SUBJECT_LABELS.get(subject, subject)}\n\n"
        f"📝 محاولات الحل: {attempts}\n"
        f"✅ الصحيحة: {correct}\n"
        f"❌ الخاطئة: {wrong}\n"
        f"📈 الدقة: {pct}%\n\n"
        f"🧠 النصيحة: {advice}"
    )
    await safe_edit(query, msg, reply_markup=pp_subject_menu_keyboard(uni, subject))
...
async def panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "support":
        ...
        return await safe_edit(query, text, reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("☎️ تواصل مع الدعم", callback_data="support:contact")],
            [InlineKeyboardButton("🚨 الإبلاغ عن مشكلة", callback_data="support:problem")],
            [InlineKeyboardButton("⬅️ رجوع", callback_data="main:home")],
        ]))
    if action == "study_now":
        uid = update.effective_user.id if update.effective_user else update.effective_chat.id
        weakest = await pp_get_weakest_section(uid)
        if not weakest:
            return await safe_edit(
                query,
                "🧠 ماذا أدرس الآن؟\n━━━━━━━━━━━━━━\n\nلم تحل أسئلة كافية بعد. ابدأ باختبار سريع حتى أحدد مستواك.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("📚 بنك السنوات السابقة", callback_data="pp:home")],
                    [InlineKeyboardButton("⬅️ الرئيسية", callback_data="main:home")],
                ])
            )
        uni = _pp_canonical_uni(weakest.get("uni"))
        subject = _pp_canonical_subject(weakest.get("subject"))
        text = (
            "🧠 ماذا أدرس الآن؟\n"
            "━━━━━━━━━━━━━━\n\n"
            "حسب نتائجك:\n"
            ...
            f"🏛️ {UNI_LABELS.get(uni, uni)}\n"
            f"📚 {SUBJECT_LABELS.get(subject, subject)}\n\n"
            f"📈 دقتك: {weakest.get('accuracy', 0)}%\n"
            f"❌ أخطاء محفوظة: {weakest.get('wrong', 0)}\n\n"
            "الخطة الآن:\n\n"
            "1) راجع أسئلتك الخاطئة\n"
            "2) حل اختبار سريع 20 سؤال\n"
            "3) إذا وصلت 80% انتقل لقسم آخر"
        )
        return await safe_edit(query, text, reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("❌ راجع أخطائي", callback_data=f"pp:wrong:{uni}:{subject}")],
            [InlineKeyboardButton("⚡ اختبار سريع", callback_data=f"pp:quiz:{uni}:{subject}")],
            [InlineKeyboardButton("⬅️ الرئيسية", callback_data="main:home")],
        ]))
    if action == "progress":
        ...
        return await safe_edit(query, msg, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ رجوع", callback_data="main:home")]]))
    ...
    if action == "subscription":
        ...
        madar = MADAR_PAYMENT_NUMBER or "لم يتم ضبط رقم المدار"
        msg = (
            "💎 الاشتراك في MedMCQ\n"
            "━━━━━━━━━━━━━━\n\n"
            f"👤 خطتك الحالية: {PLAN_LABELS.get(plan, plan)}\n"
            f"⏳ تنتهي في: {exp_txt}\n"
            f"🆔 User ID: {uid}\n\n"
            "💎 Premium شهري\n"
            "20 د.ل / 30 يوم\n"
            "• بنك السنوات السابقة كامل بدون حد\n"
            "• 3 ملخصات يوميًا\n"
            "• 3 Basic يوميًا\n"
            "• 2 Cases + 2 Challenge يوميًا\n"
            "• OSCE مرتين بالأسبوع\n\n"
            "🔥 Premium فصل كامل\n"
            "45 د.ل / 4 شهور\n"
            "• نفس Premium بسعر أوفر\n\n"
            "🚀 Premium Plus\n"
            "65 د.ل / 4 شهور\n"
            "• بنك السنوات السابقة كامل بدون حد\n"
            "• حدود أعلى\n"
            ...
            f"{madar}\n\n"
            "مهم:\n"
            "الأفضل تختار الخطة أولًا ثم تحوّل.\n"
            "ولو كنت حولت مسبقًا، اكتب نفس رقم المدار وسأحاول مطابقة التحويل تلقائيًا."
        )
        return await safe_edit(query, msg, reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("💎 Premium شهري — 20 د.ل", callback_data="payment:premium_month")],
            [InlineKeyboardButton("🔥 Premium فصل كامل / 4 شهور — 45 د.ل", callback_data="payment:premium_semester")],
            [InlineKeyboardButton("🚀 Premium Plus / 4 شهور — 65 د.ل", callback_data="payment:premium_plus_semester")],
            [InlineKeyboardButton("💎 خطتي الحالية / My Plan", callback_data="payment:my_plan")],
            [InlineKeyboardButton("☎️ الدعم", callback_data="panel:support")],
            [InlineKeyboardButton("⬅️ الرئيسية", callback_data="main:home")],
        ]))
...
async def payment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await safe_answer_query(query)
    data = query.data or "payment:request"
    uid = update.effective_user.id if update.effective_user else 0
    madar = MADAR_PAYMENT_NUMBER or "لم يتم ضبط رقم المدار"
    ...
    if data == "payment:cancel_open":
        changed = await db_cancel_pending_payment_order(uid, note="cancelled by user from payment menu")
        context.user_data.pop("awaiting_payment_phone", None)
        if changed:
            return await safe_edit(query, "✅ تم إلغاء طلب الدفع المفتوح.\nيمكنك الآن اختيار خطة جديدة.", reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("💎 الاشتراكات", callback_data="panel:subscription")],
                [InlineKeyboardButton("⬅️ الرئيسية", callback_data="main:home")],
            ]))
        return await safe_edit(query, "لا يوجد طلب دفع مفتوح حاليًا.", reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("💎 الاشتراكات", callback_data="panel:subscription")],
            [InlineKeyboardButton("⬅️ الرئيسية", callback_data="main:home")],
        ]))

    if data == "payment:my_plan":
        info = await db_get_user_plan_info(uid)
        plan = info.get("plan", "free")
        exp = float(info.get("plan_expires_at") or 0)
        exp_txt = "لا يوجد" if not exp else datetime.fromtimestamp(exp).strftime("%Y-%m-%d")
        msg = (
            "💎 خطة حسابك\n\n"
            f"User ID:\n`{uid}`\n\n"
            f"الخطة الحالية: {PLAN_LABELS.get(plan, plan)}\n"
            f"تنتهي في: {exp_txt}"
        )
        return await safe_edit(query, msg, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("💎 رجوع للاشتراكات", callback_data="panel:subscription")],
            [InlineKeyboardButton("⬅️ الرئيسية", callback_data="main:home")],
        ]))

    payment_plan_codes = {
        "payment:premium_month": "premium_month",
        "payment:premium_semester": "premium_semester",
        "payment:premium_plus_semester": "premium_plus_semester",
    }

    if data not in payment_plan_codes:
        return await safe_edit(query, "اختر خطة الاشتراك أولاً.", reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("💎 الاشتراكات", callback_data="panel:subscription")],
            [InlineKeyboardButton("⬅️ الرئيسية", callback_data="main:home")],
        ]))

    open_order = await db_get_open_pending_payment_order(uid)
    if open_order:
        mins_left = int(max(0, (float(open_order.get("expires_at", 0)) - time.time()) // 60))
        return await safe_edit(
            query,
            "لديك طلب دفع مفتوح بالفعل. أكمله أو ألغيه أولًا.\n\n"
            f"رقم الطلب: `{open_order.get('order_code')}`\n"
            f"الخطة: {open_order.get('plan_title')}\n"
            f"المبلغ: {int(float(open_order.get('amount') or 0))} د.ل\n"
            f"رقم المحوّل: `{open_order.get('payer_phone')}`\n"
            f"ينتهي بعد: {mins_left} دقيقة",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("❌ إلغاء الطلب الحالي", callback_data="payment:cancel_open")],
                [InlineKeyboardButton("⬅️ الاشتراكات", callback_data="panel:subscription")],
            ]),
        )

    plan_code = payment_plan_codes[data]
    plan = payment_plan_details(plan_code, uid)
    context.user_data["awaiting_payment_phone"] = {"plan_code": plan_code, **plan}

    msg = (
        "📱 اكتب رقم المدار الذي ستحوّل منه\n\n"
        f"الخطة: {plan['title']}\n"
        f"المبلغ: {int(plan['amount'])} د.ل\n"
        f"رقم المدار الذي ستحول إليه:\n`{madar}`\n\n"
        "اكتب رقمك الآن في خانة الرد تحت هذه الرسالة.\n\n"
        "مثال:\n"
        "`0912345678`\n"
        "أو\n"
        "`0932345678`\n\n"
        "⚠️ اكتب الرقم فقط بدون كلام إضافي."
    )

    await safe_edit(
        query,
        "✅ تم اختيار الخطة.\n\n"
        "سأرسل لك الآن رسالة خاصة لإدخال رقم المدار.\n"
        "اكتب الرقم في خانة الرد التي ستظهر لك.",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("💎 رجوع للاشتراكات", callback_data="panel:subscription")],
            [InlineKeyboardButton("⬅️ الرئيسية", callback_data="main:home")],
    ...
    return await query.message.reply_text(
        ...
        reply_markup=ForceReply(
            selective=True,
            input_field_placeholder="اكتب رقم المدار هنا..."
        )
    )
...
async def set_plan_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        return await update.message.reply_text(
            ...
            "الأسعار المعتمدة:\n"
            "Premium شهري: 20 د.ل\n"
            "Premium فصل كامل / 4 شهور: 45 د.ل\n"
            "Premium Plus فصل كامل / 4 شهور: 65 د.ل"
        )
    try:
...
async def ai_policy_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "الترتيب: Google → OpenRouter Free → كاش → Groq Free → Cloudflare Free → Paid فقط للأدمن/بريميوم حسب الإعداد."
    )
...
async def support_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid = update.effective_user.id if update.effective_user else 0
    madar = MADAR_PAYMENT_NUMBER or "لم يتم ضبط رقم المدار"
    data = query.data or "support:home"

    if data in {"support:home", "support:balance"}:
        text = (
            "☎️ الدعم\n\n"
            "اختر ما تريد:\n\n"
            "• تواصل مع الدعم: لإرسال إثبات الدفع أو الاستفسار.\n"
    ...
    if data == "support:contact":
        ...
        text = (
            "☎️ تواصل مع الدعم\n\n"
            f"لو تريد الاشتراك، حوّل على رقم المدار:\n{madar}\n\n"
            f"User ID الخاص بك:\n{uid}\n\n"
            "الآن أرسل إثبات الدفع أو رسالتك هنا، وسأرسلها للأدمن."
        )
        return await safe_edit(query, text, reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("💎 الاشتراكات", callback_data="panel:subscription")],
            [InlineKeyboardButton("🔙 رجوع للدعم", callback_data="support:home")],
        ]))

    if data == "support:problem":
        context.user_data["awaiting_support_message"] = "problem"
        text = (
            "🚨 الإبلاغ عن مشكلة\n\n"
            f"User ID الخاص بك:\n{uid}\n\n"
            "اكتب المشكلة الآن، ويفضل ترسل صورة شاشة لو موجودة."
        )
        return await safe_edit(query, text, reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔙 رجوع للدعم", callback_data="support:home")],
            [InlineKeyboardButton("🏠 الرئيسية", callback_data="main:home")],
        ]))
...
async def admin_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "home":
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("📤 رفع ملف JSON جاهز", callback_data="admin:add")],
    ...
    if action == "add":
        ...
        return await safe_edit(query, "📤 رفع ملف JSON جاهز\n\nاختر الجامعة:", reply_markup=kb)
    if action == "adduni" and len(parts) >= 3:
        uni = _pp_canonical_uni(parts[2])
        ...
        return await safe_edit(query, f"📤 رفع ملف JSON جاهز\n🏛️ {UNI_LABELS.get(uni, uni)}\n\nاختر المادة:", reply_markup=kb)
    if action == "addsubject" and len(parts) >= 4:
        ...
        return await safe_edit(
            query,
            f"✅ وضع رفع ملف JSON جاهز\n\n🏛️ {UNI_LABELS.get(uni, uni)}\n📚 {SUBJECT_LABELS.get(subject, subject)}\n\nأرسل الآن ملف JSON فقط لهذا القسم.\nسيتم حفظه مباشرة في جدول past_papers، وهو نفس بنك الأسئلة الذي يقرأ منه تدريب عشوائي وحل بالترتيب.\n\nللصور/PDF/Word/النص الخام استخدم زر: ⚡ جمع سنوات سابقة سريع.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🛠️ رجوع للوحة الأدمن", callback_data="admin:home")]])
        )
    if action == "dumpzip":
...
async def payment_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid not in ADMIN_USER_IDS:
        return await safe_edit(query, "⛔ هذا الزر للإدارة فقط.")
    data = query.data or ""
    parts = data.split(":")
    if len(parts) < 3:
        return await safe_edit(query, "⚠️ زر دفع غير صالح.")
    action, order_id = parts[1], int(parts[2])
    order = await db_get_payment_order(order_id)
    if not order:
        return await safe_edit(query, "⚠️ طلب الدفع غير موجود.")
    if action == "approve":
        result = await db_manual_approve_payment_order(order_id, admin_id=uid)
        if not result.get("ok"):
            return await safe_edit(query, f"⚠️ لم يتم التفعيل. الحالة: {result.get('status')}")
        order = result.get("order") or order
        try:
            await context.bot.send_message(
                chat_id=int(order["user_id"]),
                text=f"✅ تم تفعيل اشتراكك يدويًا.\n\nالخطة: {order['plan_title']}\nالمدة: {order['days']} يوم"
            )
        except Exception:
            pass
        return await safe_edit(query, f"✅ تم تفعيل الاشتراك.\nUser ID: {order['user_id']}\nالخطة: {order['plan_title']}")
    if action == "reject":
        await db_mark_payment_order_status(order_id, "rejected", note=f"rejected by admin {uid}")
        try:
            await context.bot.send_message(
                chat_id=int(order["user_id"]),
                text="❌ لم يتم قبول طلب الدفع. إذا كنت متأكدًا أنك حولت، تواصل مع الدعم وأرسل التفاصيل."
            )
        except Exception:
            pass
        return await safe_edit(query, f"❌ تم رفض الطلب.\nUser ID: {order['user_id']}\nالخطة: {order['plan_title']}")
    if action == "wait":
        return await safe_edit(query, "🕓 تم ترك الطلب pending للمراجعة لاحقًا.\nلم يتم تفعيل أو رفض الطالب.")
    return await safe_edit(query, "⚠️ إجراء غير معروف.")
...
async def universal_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        elif data.startswith("payadmin:"):
            await payment_admin_callback(update, context)
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. مسار رفع ملف JSON جاهز للأدمن — لا يدخل staging الخام
    pf = context.user_data.get("pending_final")
    if user_id in ADMIN_USER_IDS and pf:
        file_name = doc.file_name or "uploaded.json"
        if not file_name.lower().endswith(".json"):
            return await update.message.reply_text(
                "❌ أرسل ملف JSON فقط لهذا الزر.\n"
                "زر 📤 رفع ملف JSON جاهز لا يستقبل صور/PDF/Word.\n"
                "للصور والملفات الخام استخدم: ⚡ جمع سنوات سابقة سريع."
            )

        tg_file = await doc.get_file()
        raw_bytes = bytes(await tg_file.download_as_bytearray())
        try:
            json_text = raw_bytes.decode("utf-8-sig", errors="strict").strip()
        ...
        try:
            ...
            return await update.message.reply_text(f"❌ ملف JSON غير صالح. التفاصيل: {e}")

        parsed_data = _pp_extract_json_question_list(parsed_obj)
        if not isinstance(parsed_data, list) or not parsed_data:
            return await update.message.reply_text(
                "❌ هيكلية JSON غير مدعومة.\n"
                "الصيغ المقبولة: list مباشرة أو قاموس يحتوي questions/items/data/mcqs."
            )

        uni = _pp_canonical_uni(pf.get("uni"))
        subject = _pp_canonical_subject(pf.get("subject"))
        new_questions, duplicate_count, invalid_count = await pp_filter_new_questions(uni, subject, parsed_data)
            ...
            async with _db_lock:
                def w():
                    ...
                    conn.execute(
                        ...
                        (uni, subject, json.dumps(new_questions, ensure_ascii=False), datetime.utcnow().isoformat())
                    )
                    conn.commit()
                    conn.close()
                await asyncio.to_thread(w)

        total_now = len(await pp_load_questions(uni, subject))
        saved = len(new_questions)
        total_input = len(parsed_data)
        context.user_data.pop("pending_final", None)

        await update.message.reply_text(
            "✅ تم رفع ملف JSON الجاهز وتخزينه في بنك الأسئلة\n\n"
            f"القسم: {uni} / {subject}\n"
            f"الجديد المضاف: {saved}\n"
            f"إجمالي أسئلة القسم الآن: {total_now}\n\n"
            f"📦 إجمالي الملف: {total_input}\n"
            f"♻️ مكرر لم يُضاف: {duplicate_count}\n"
            f"⚠️ غير صالح/بدون إجابة مؤكدة: {invalid_count}\n\n"
            "الآن جرّب زر تدريب عشوائي في نفس القسم."
        )
        return
    ...
    try:
        ...
        if mode == "summary":
            ...
            if cached_summary:
                await status.delete()
                for c in split_telegram_text(format_summary_for_student(cached_summary, style)):
            ...
            await status.delete()
            if summary:
                for c in split_telegram_text(format_summary_for_student(summary, style)):
                    await update.message.reply_text(c)
                await increment_daily_usage(context.user_data, "summary", user.id if user else 0)
                if user:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pending_payment = context.user_data.get("awaiting_payment_phone")
    if pending_payment and text and not text.startswith('/'):
        payer_phone = normalize_madar_phone(text)
        if not ((payer_phone.startswith("091") or payer_phone.startswith("093")) and len(payer_phone) == 10):
            return await update.message.reply_text(
                "⚠️ الرقم غير صحيح.\n\n"
                "اكتب رقم المدار الذي ستحوّل منه بصيغة صحيحة:\n"
                "`091xxxxxxx` أو `093xxxxxxx`\n\n"
                "اكتب الرقم فقط بدون كلام إضافي.",
                parse_mode="Markdown",
                reply_markup=ForceReply(
                    selective=True,
                    input_field_placeholder="اكتب رقم المدار هنا..."
        ...
        context.user_data.pop("awaiting_payment_phone", None)
        created = await db_create_payment_order(
            user_id=user.id if user else 0,
            plan=pending_payment["plan"],
            days=int(pending_payment["days"]),
            plan_title=pending_payment["title"],
            amount=float(pending_payment["amount"]),
            payer_phone=payer_phone,
        )
        if not created.get("ok") and created.get("open_pending"):
            existing = created.get("existing") or {}
            mins_left = int(max(0, (float(existing.get("expires_at", 0)) - time.time()) // 60))
            context.user_data.pop("awaiting_payment_phone", None)
            return await update.message.reply_text(
                "لديك طلب دفع مفتوح بالفعل. أكمله أو ألغيه أولًا.\n\n"
                f"رقم الطلب: `{existing.get('order_code')}`\n"
                f"الخطة: {existing.get('plan_title')}\n"
                f"المبلغ: {int(float(existing.get('amount') or 0))} د.ل\n"
                f"رقم المحوّل: `{existing.get('payer_phone')}`\n"
                f"ينتهي بعد: {mins_left} دقيقة",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ إلغاء الطلب الحالي", callback_data="payment:cancel_open")]])
            )
        if not created.get("ok") and created.get("conflict"):
            return await update.message.reply_text(
                "⚠️ يوجد طلب دفع مفتوح بنفس رقم المدار ونفس المبلغ.\n"
                "انتظر حتى يكتمل الطلب السابق أو استخدم رقمًا آخر أو تواصل مع الدعم."
            )
        order = created["order"]
        auto_from_stored = await try_auto_approve_order_from_stored_sms(order)
        if auto_from_stored:
            await send_admin_log(
                context,
                "✅ تم ربط تحويل مدار سابق تلقائيًا بطلب جديد\n\n"
            ...
            return await update.message.reply_text(
                "✅ تم العثور على تحويل مدار سابق مطابق وتفعيل اشتراكك تلقائيًا.\n\n"
                f"الخطة: {order['plan_title']}\n"
                f"المدة: {order['days']} يوم\n"
                f"المبلغ: {int(float(order['amount']))} د.ل",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💎 خطتي الحالية", callback_data="payment:my_plan")]])
            )
        minutes = int(max(1, (float(order["expires_at"]) - time.time()) // 60))
        await send_admin_log(
            context,
            "🧾 طلب دفع جديد بانتظار رسالة المدار\n\n"
            f"Order: {order['order_code']}\n"
            f"User ID: {order['user_id']}\n"
        ...
        return await update.message.reply_text(
            "✅ تم إنشاء طلب الدفع.\n\n"
            f"رقم الطلب: `{order['order_code']}`\n"
            f"الخطة: {order['plan_title']}\n"
            f"المبلغ المطلوب: `{int(float(order['amount']))} د.ل`\n"
            f"حوّل من الرقم: `{order['payer_phone']}`\n"
            f"إلى رقم المدار: `{MADAR_PAYMENT_NUMBER}`\n\n"
            f"⏳ الطلب صالح لمدة {minutes} دقيقة.\n"
            "بعد التحويل لا تحتاج إرسال صورة. عندما تصل رسالة المدار الرسمية سيتم تفعيل الاشتراك تلقائيًا.\n\n"
            "إذا لم يتفعل خلال 10 دقائق، افتح الدعم وأرسل رقم الطلب.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("☎️ الدعم", callback_data="panel:support")],
                [InlineKeyboardButton("💎 الاشتراكات", callback_data="panel:subscription")],
...
# =============================================================================
# PAYMENT WATCHDOG ADMIN COMMANDS
# =============================================================================

def _payment_human_time_ago(ts: float) -> str:
    try:
        diff = max(0, int(time.time() - float(ts)))
    except Exception:
        return "غير معروف"
    if diff < 60:
        return f"{diff} ثانية"
...
def _read_payment_phone_heartbeat() -> dict:
    try:
        fp = os.environ.get("PAYMENT_PHONE_HEARTBEAT_FILE", "data/payment_phone_heartbeat.json")
...
async def db_list_pending_payment_orders(limit: int = 20) -> list:
    now = time.time()
    def work():
        conn = _db_connect()
        conn.execute(
            "UPDATE payment_orders SET status='expired', note='auto expired' "
...
async def payment_status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id if update.effective_user else 0
    if user_id not in ADMIN_USER_IDS:
        return await update.message.reply_text("⛔ هذا الأمر للإدارة فقط.")

    hb = _read_payment_phone_heartbeat()
    if hb and hb.get("ts"):
        age = _payment_human_time_ago(float(hb.get("ts", 0)))
        phone_status = f"✅ هاتف الدفع آخر إشارة: منذ {age}"
    else:
        phone_status = "⚠️ لا توجد إشارة مسجلة من هاتف الدفع"

    pending = await db_list_pending_payment_orders(limit=50)
    recent_sms = await db_list_recent_madar_sms(limit=5)

    text = (
        "📡 حالة نظام الدفع\n\n"
        f"{phone_status}\n"
        f"📋 طلبات الدفع المعلقة: {len(pending)}\n"
        f"📨 آخر رسائل مدار محفوظة: {len(recent_sms)}\n\n"
        "الأوامر:\n"
        "/pending_payments لعرض الطلبات المعلقة\n"
        "/payment_audit لعرض آخر رسائل المدار"
    )
...
async def pending_payments_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = await db_list_pending_payment_orders(limit=10)
    if not rows:
        return await update.message.reply_text("✅ لا توجد طلبات دفع معلقة حاليًا.")

    await update.message.reply_text(f"📋 طلبات الدفع المعلقة: {len(rows)}")

    for r in rows:
        age = _payment_human_time_ago(float(r.get("created_at", 0)))
        mins_left = int(max(0, (float(r.get("expires_at", 0)) - time.time()) // 60))
        near = await db_find_partial_sms_for_order(r, limit=3)
        if near:
            near_lines = []
            for sms in near:
                flags = []
                if normalize_madar_phone(sms.get("parsed_phone", "")) == normalize_madar_phone(r.get("payer_phone", "")):
                    flags.append("رقم مطابق")
                if sms.get("parsed_amount") is not None and abs(float(sms.get("parsed_amount") or 0) - float(r.get("amount") or 0)) < 0.01:
                    flags.append("مبلغ مطابق")
                near_lines.append(
                    f"• SMS #{sms.get('id')}: {sms.get('parsed_phone') or '-'} / {sms.get('parsed_amount') or '-'} د.ل / {sms.get('status')} ({' + '.join(flags) or 'جزئي'})"
                )
            near_txt = "\n".join(near_lines)
        else:
            near_txt = "لا يوجد SMS قريب خلال آخر 24 ساعة"
        text = (
            f"🧾 طلب دفع معلق #{r['id']}\n\n"
            f"Order: {r.get('order_code')}\n"
            f"User ID: {r.get('user_id')}\n"
            f"الخطة: {r.get('plan_title')}\n"
            f"المبلغ: {int(float(r.get('amount', 0)))} د.ل\n"
            f"رقم المحوّل: {r.get('payer_phone')}\n"
            f"منذ: {age}\n"
            f"ينتهي بعد: {mins_left} دقيقة\n\n"
            f"هل يوجد SMS قريبة؟\n{near_txt}"
        )
        kb = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("✅ فعّل", callback_data=f"payadmin:approve:{r['id']}"),
                InlineKeyboardButton("❌ ارفض", callback_data=f"payadmin:reject:{r['id']}"),
                InlineKeyboardButton("🕓 انتظر", callback_data=f"payadmin:wait:{r['id']}")
            ]
        ])
        await update.message.reply_text(text, reply_markup=kb)


async def payment_audit_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id if update.effective_user else 0
    if user_id not in ADMIN_USER_IDS:
        return await update.message.reply_text("⛔ هذا الأمر للإدارة فقط.")
    rows = await db_list_recent_madar_sms(limit=20)
    if not rows:
        return await update.message.reply_text("لا توجد رسائل مدار محفوظة في السجل بعد.")
    lines = ["🧾 Payment Safety Audit — آخر 20 SMS من Almadar", ""]
    for r in rows:
        age = _payment_human_time_ago(float(r.get("received_at", 0)))
        claimed = "لا"
        if int(r.get("claimed_order_id") or 0):
            claimed = f"نعم، Order #{r.get('claimed_order_id')} / User {r.get('claimed_user_id')}"
        lines.append(
            f"#{r.get('id')} | {r.get('parsed_phone') or '-'} | {r.get('parsed_amount') if r.get('parsed_amount') is not None else '-'} د.ل | {r.get('status')} | منذ {age}\n"
            f"الرابط: {claimed}"
        )
    msg = "\n\n".join(lines)
...
BOT_COMMANDS = [
    ...
    BotCommand("payment_status", "حالة الدفع (أدمن)"),
    BotCommand("pending_payments", "طلبات الدفع المعلقة (أدمن)"),
    BotCommand("payment_audit", "تدقيق رسائل المدار (أدمن)")
]
...
def main():
    ...
    application.add_handler(CommandHandler("payment_status", payment_status_cmd))
    application.add_handler(CommandHandler("pending_payments", pending_payments_cmd))
    application.add_handler(CommandHandler("payment_audit", payment_audit_cmd))
