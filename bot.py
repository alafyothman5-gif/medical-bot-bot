...
MADAR_PAYMENT_NUMBER = os.environ.get("MADAR_PAYMENT_NUMBER", os.environ.get("ALMADAR_PHONE", "")).strip()
PLAN_PRICES_AR = {
...
async def payment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    plans = {
        "payment:premium_month": ("Premium شهري", "20 د.ل", f"/set_plan {uid} premium 30"),
        "payment:premium_semester": ("Premium فصل كامل", "45 د.ل", f"/set_plan {uid} premium 120"),
        "payment:premium_plus_semester": ("Premium Plus فصل كامل", "65 د.ل", f"/set_plan {uid} premium_plus 120"),
    }
    ...
    plan_title, price, command_hint = plans[data]
    await db_create_payment_request(uid, f"Requested {plan_title} {price}")
    await send_admin_log(context, f"💎 طلب اشتراك\nالخطة: {plan_title}\nالسعر: {price}\nUser ID: {uid}\nName: {update.effective_user.full_name if update.effective_user else ''}")

    msg = (
        "🧾 طلب اشتراك جاهز\n\n"
        f"الخطة المطلوبة: {plan_title}\n"
        f"السعر: {price}\n\n"
        f"رقم المدار للتحويل:\n`{madar}`\n\n"
        f"User ID الخاص بك:\n`{uid}`\n\n"
        "بعد التحويل:\n"
        "1) صوّر إثبات الدفع.\n"
        "2) اضغط تواصل مع الدعم.\n"
        "3) أرسل الإثبات ومعه User ID.\n\n"
        "للأدمن عند التفعيل:\n"
        f"`{command_hint}`"
    )
    await safe_edit(query, msg, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton("☎️ إرسال الإثبات للدعم", callback_data="support:contact")],
        [InlineKeyboardButton("💎 رجوع للاشتراكات", callback_data="panel:subscription")],
        [InlineKeyboardButton("⬅️ الرئيسية", callback_data="main:home")],
    ]))
