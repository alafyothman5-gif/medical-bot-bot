        ...
        for key_idx, api_key in enumerate(GOOGLE_API_KEYS, 1):
            try:
                ...
                if page_text and len(page_text.strip()) >= 20:
                    logger.info("Admin page vision OK file=%s page=%s key=%s/%s", file_name, page_idx, key_idx, len(GOOGLE_API_KEYS))
                    break
            except Exception as e:
                last_error = e
                logger.warning("Admin page vision failed file=%s page=%s key=%s/%s: %s", file_name, page_idx, key_idx, len(GOOGLE_API_KEYS), str(e)[:180])
        if page_text:
            texts.append(f"\n--- Page/Slide {page_idx} ---\n{page_text.strip()}")
        elif last_error:
            logger.warning("Admin page vision all keys failed file=%s page=%s: %s", file_name, page_idx, last_error)
    return "\n".join(texts).strip()
