...
SMS_WEBHOOK_TOKEN = os.environ.get("SMS_WEBHOOK_TOKEN", "CHANGE_ME_SMS_TOKEN").strip()
...
def payment_plan_details(plan_code: str, user_id: int) -> Optional[dict]:
    plans = {
        ...
        "premium_semester": {"title": "Premium فصل كامل", "amount": 45.0, "plan": "premium", "days": 120},
        "premium_plus_semester": {"title": "Premium Plus فصل كامل", "amount": 65.0, "plan": "premium_plus", "days": 120},
    }
...
async def db_record_processed_sms(sms_text: str, phone: str = "", amount: Optional[float] = None, result: str = "received") -> bool:
    """Return True if this SMS was new, False if duplicate."""
...
async def panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "progress":
        ...
        msg = (
            ...
            "🚀 Premium Plus فصل كامل — 65 د.ل / 4 شهور\n"
            "• 5 ملخصات يومياً\n"
            "• 5 Basic يومياً\n"
        ...
        await safe_edit(query, msg, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🔥 Premium فصل كامل — 45 د.ل", callback_data="payment:premium_semester")],
            [InlineKeyboardButton("🚀 Premium Plus فصل كامل — 65 د.ل", callback_data="payment:premium_plus_semester")],
...
async def payment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        "📱 خطوة واحدة قبل التحويل\n\n"
        f"الخطة: {plan['title']}\n"
        f"المبلغ: {int(plan['amount'])} د.ل\n"
        f"رقم المدار الذي ستحول إليه:\n`{madar}`\n\n"
        "اكتب الآن رقم المدار الذي ستحوّل منه أنت، بصيغة:\n"
        "`091xxxxxxx` أو `093xxxxxxx`\n\n"
        "بعدها سيعطيك البوت طلب دفع رسمي. عند وصول رسالة المدار إلى هاتفي سيتم التفعيل تلقائيًا."
    )
    return await safe_edit(query, msg, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton("💎 رجوع للاشتراكات", callback_data="panel:subscription")],
        [InlineKeyboardButton("⬅️ الرئيسية", callback_data="main:home")],
...
async def set_plan_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        return await update.message.reply_text(
            ...
            "الأسعار المعتمدة:\n"
            "Premium شهري: 20 د.ل\n"
            "Premium فصل كامل: 45 د.ل\n"
            "Premium Plus فصل كامل: 65 د.ل"
        )
    try:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pending_payment and text and not text.startswith('/'):
        ...
        if not ((payer_phone.startswith("091") or payer_phone.startswith("093")) and len(payer_phone) == 10):
            return await update.message.reply_text(
                "⚠️ الرقم غير صحيح. اكتب رقم المدار الذي ستحول منه بصيغة:\n"
                "091xxxxxxx أو 093xxxxxxx"
            )
