...
PREMIUM_ENABLED = _env_bool("PREMIUM_ENABLED", "0")
...
async def db_get_user_plan(user_id: int) -> str:
    ...
    def work():
        conn = _db_connect()
        row = conn.execute("SELECT plan FROM users WHERE user_id=?", (int(user_id),)).fetchone()
        conn.close()
...
async def db_set_user_plan(user_id: int, plan: str) -> None:
    plan = (plan or "free").strip().lower()
    if plan not in {"free", "premium", "admin"}: plan = "free"
    async with _db_lock:
        def work():
            now = datetime.utcnow().isoformat()
            conn = _db_connect()
            conn.execute("INSERT INTO users(user_id, plan, created_at, last_seen) VALUES(?,?,?,?) ON CONFLICT(user_id) DO UPDATE SET plan=excluded.plan, last_seen=excluded.last_seen", (int(user_id), plan, now, now))
...
async def set_plan_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if plan not in {"free", "premium", "admin"}:
        return await update.message.reply_text("⚠️ الخطة لازم تكون: free أو premium أو admin")
    await db_set_user_plan(target_id, plan)
    await update.message.reply_text(f"✅ تم ضبط خطة المستخدم {target_id} إلى: {plan}")
...
async def ai_policy_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "الترتيب: كاش → Google → OpenRouter Free → Groq Free → Paid فقط للأدمن/بريميوم حسب الإعداد."
    )
