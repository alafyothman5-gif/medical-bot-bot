...
GOOGLE_API_KEYS = [k.strip() for k in re.split(r"[,;\s]+", os.environ.get("GOOGLE_API_KEYS", os.environ.get("GEMINI_API_KEYS", os.environ.get("GOOGLE_API_KEY", os.environ.get("GEMINI_API_KEY", ""))))) if k.strip()]
...
def init_db() -> None:
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS user_usage_limits (user_id INTEGER NOT NULL, kind TEXT NOT NULL, last_used_at REAL NOT NULL, PRIMARY KEY (user_id, kind))")
...
def _usage_interval_for(kind: str) -> int:
    return int(FREE_USAGE_INTERVALS.get(kind, 0))

async def check_daily_limit(update: Update, context: ContextTypes.DEFAULT_TYPE, kind: str) -> bool:
    """Economy limits for free users. Cached results should call this only when AI generation is needed."""
    ...
    if not ECONOMY_MODE:
        return True
    interval = _usage_interval_for(kind)
    if interval <= 0:
        return True
    now = time.time()
    ...
    def work():
        conn = _db_connect()
        row = conn.execute("SELECT last_used_at FROM user_usage_limits WHERE user_id=? AND kind=?", (int(user_id), kind)).fetchone()
        conn.close()
        return float(row["last_used_at"]) if row else 0.0

    last_used = await asyncio.to_thread(work)
    remaining = int(interval - (now - last_used))
    if last_used and remaining > 0:
        ...
        await update.effective_message.reply_text(
            f"⚠️ وصلت لحد الخطة المجانية في: {labels.get(kind, kind)}.\n"
...
async def increment_daily_usage(user_data: dict, kind: str, user_id: Optional[int] = None) -> None:
    ...
    async with _db_lock:
        def work():
            conn = _db_connect()
            conn.execute(
                "INSERT OR REPLACE INTO user_usage_limits(user_id, kind, last_used_at) VALUES(?,?,?)",
                (int(user_id), kind, time.time()),
            )
...
async def _call_google_vision(image_bytes: bytes, api_key: str) -> str:
    ...
    payload = {
        "contents": [{
            "role": "user",
            "parts": [
                {"text": "Extract all medical exam questions, text, options, and answers from this image clearly. If the text is in Arabic, extract it in Arabic."},
...
async def _call_openrouter_text(prompt: str, temperature: float, max_output_tokens: int) -> str:
    if not OPENROUTER_API_KEY:
        raise Exception("OPENROUTER_API_KEY is missing and Google direct failed/unavailable.")
...
async def _call_openrouter_vision(image_bytes: bytes) -> str:
    if not OPENROUTER_API_KEY:
        raise Exception("OPENROUTER_API_KEY is missing and Google direct vision failed/unavailable.")
    base64_image = base64.b64encode(image_bytes).decode('utf-8')
    payload = {
...
async def call_gemini(prompt: str, temperature: float = 0.2, max_output_tokens: int = 4096) -> str:
    """Use Google Gemini direct keys first; fallback to OpenRouter only if all Google keys fail."""
    ...
    async with _GEMINI_SEMAPHORE:
        ...
        try:
            if GOOGLE_API_KEYS:
                ...
                logger.warning("All Google keys failed, falling back to OpenRouter. Last error: %s", last_error)
            result = await _call_openrouter_text(prompt, temperature, max_output_tokens)
            logger.info("AI provider: OpenRouter fallback model=%s", GEMINI_MODEL_NAME)
            return result
        finally:
            async with _AI_QUEUE_LOCK:
...
async def call_gemini_vision(image_bytes: bytes) -> str:
    """Vision uses Google direct first, then OpenRouter fallback."""
    if GOOGLE_API_KEYS:
        last_error = None
        ...
        logger.warning("All Google vision keys failed, falling back to OpenRouter. Last error: %s", last_error)
    try:
        text = await _call_openrouter_vision(image_bytes)
        logger.info("Vision provider: OpenRouter fallback model=%s", GEMINI_MODEL_NAME)
        return text
    except Exception as e:
...
async def handle_admin_preload_file(update: Update, context: ContextTypes.DEFAULT_TYPE, raw_bytes: bytes, file_name: str, mime_type: str, status_message=None):
    ...
    if file_type == "unsupported":
        return await status.edit_text("⚠️ نوع الملف غير مدعوم للتخزين. أرسل PDF/DOCX/PPTX/TXT.")
    ...
    if not text or len(text.strip()) < 100:
        return await status.edit_text("⚠️ لم أستطع استخراج نص واضح من هذا الملف.")
    raw_hash = sha256_bytes(raw_bytes)
    content_hash = canonical_text_hash(text)
    existing = await db_get_file(content_hash)
    if not existing:
        await db_save_file(content_hash, file_name or "lecture", len(raw_bytes), mime_type or "", text)
    # Save raw hash too, so exact same file is found quickly later.
    if not await db_get_file(raw_hash):
        await db_save_file(raw_hash, file_name or "lecture", len(raw_bytes), mime_type or "", text)
    qsize, job_id = await enqueue_admin_preload_jobs(update.effective_chat.id, update.effective_user.id if update.effective_user else 0, content_hash, text, file_name or "lecture")
    if job_id:
        return await status.edit_text(
            "✅ تم تخزين الملف في مخزون الفصل.\n"
            f"📄 الملف: {file_name}\n"
            f"🧠 النص المستخدم للـ AI: حتى {MAX_TEXT_CHARS_FOR_AI} حرف بتوزيع بداية/وسط/نهاية.\n"
            f"📥 دخل تجهيز الملخصات والأسئلة في الخلفية.\n"
            f"📌 Job ID: {job_id}\n"
            f"📊 الطابور الآن: {qsize}"
        )
    # Fallback direct preload if Redis is unavailable.
    await status.edit_text("⚠️ الطابور غير متاح. سأجهز الكاش مباشرة وقد يأخذ وقتاً...")
...
async def admin_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "preload":
        ...
        text = (
            ...
            "أرسل الآن ملفات المحاضرات واحداً واحداً.\n"
            "الحد الأقصى للملف: 30MB.\n"
