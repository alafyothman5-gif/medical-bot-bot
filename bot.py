...
async def db_store_unmatched_madar_sms(phone: str, amount: float, sms_text: str) -> None:
    ...
    async with _db_lock:
        def work():
            conn = _db_connect()
            conn.execute("DELETE FROM unmatched_madar_sms WHERE received_at < ? OR status != 'new'", (cutoff,))
            conn.execute(
                "INSERT INTO unmatched_madar_sms(phone, amount, sms_text, received_at, status) VALUES(?,?,?,?, 'new')",
                (phone, float(amount), sms_text[:4000], time.time()),
            )
            conn.commit(); conn.close()
        await asyncio.to_thread(work)
...
async def db_find_unmatched_madar_sms(phone: str, amount: float) -> Optional[dict]:
    ...
    def work():
        ...
        conn.execute("DELETE FROM unmatched_madar_sms WHERE received_at < ? OR status != 'new'", (cutoff,))
        row = conn.execute(
            "SELECT * FROM unmatched_madar_sms WHERE status='new' AND phone=? AND ABS(amount-?) < 0.01 AND received_at>=? ORDER BY received_at DESC LIMIT 1",
...
async def db_mark_unmatched_madar_sms_used(row_id: int, order_id: int) -> None:
    async with _db_lock:
        def work():
            conn = _db_connect()
            conn.execute("UPDATE unmatched_madar_sms SET status='used', used_order_id=? WHERE id=?", (int(order_id), int(row_id)))
            conn.commit(); conn.close()
        await asyncio.to_thread(work)
...
async def try_auto_approve_order_from_stored_sms(order: dict) -> bool:
    if not order:
        return False
    sms_row = await db_find_unmatched_madar_sms(order.get("payer_phone", ""), float(order.get("amount") or 0))
    if not sms_row:
        return False
    await db_set_user_plan(int(order["user_id"]), order["plan"], int(order["days"]))
    await db_mark_payment_order_status(int(order["id"]), "auto_approved", matched_sms=sms_row.get("sms_text", ""), note="auto approved from stored unmatched Almadar SMS")
    await db_mark_unmatched_madar_sms_used(int(sms_row["id"]), int(order["id"]))
    return True
...
def extract_madar_transfer_sms(text: str) -> dict:
    ...
    # Ignore balance section when choosing transfer amount.
    ...
    if not amount_matches:
        amount_matches = re.findall(r"(?:تحويل|حول|استلام|received|transfer)\D{0,20}(\d+(?:\.\d+)?)", amount_area, flags=re.I)
    amount = None
    if amount_matches:
        try:
            ...
            amount = None
    phone_matches = re.findall(r"(?:218(?:91|93)\d{7}|0(?:91|93)\d{7}|(?:91|93)\d{7})", cleaned)
...
async def db_create_payment_order(user_id: int, plan: str, days: int, plan_title: str, amount: float, payer_phone: str) -> dict:
    ...
    async with _db_lock:
        def work():
            ...
            # Avoid two open same-phone/same-amount orders at the same time.
            ...
            # Cancel old pending orders for same user to avoid confusion.
            conn.execute("UPDATE payment_orders SET status='cancelled', note='replaced by new order' WHERE user_id=? AND status='pending'", (int(user_id),))
            conn.execute(
...
async def db_record_processed_sms(sms_text: str, phone: str = "", amount: Optional[float] = None, result: str = "received") -> bool:
    """
    Return True if this SMS is new.
    Duplicate protection lasts only SMS_DUPLICATE_TTL_DAYS days.
    After that, the same SMS text is allowed again so renewals don't get blocked.
    """
    sms_hash = hashlib.sha256((sms_text or "").encode("utf-8", errors="ignore")).hexdigest()
    ttl_days = float(os.environ.get("SMS_DUPLICATE_TTL_DAYS", "14") or 14)
    ...
    async with _db_lock:
        def work():
            ...
            # احذف بصمات الرسائل القديمة حتى لا تمنع التجديد بعد أسبوعين
            conn.execute("DELETE FROM processed_sms WHERE COALESCE(processed_at, 0) < ?", (cutoff,))

            old = conn.execute(
                "SELECT sms_hash, processed_at FROM processed_sms WHERE sms_hash=?",
                (sms_hash,)
            ).fetchone()
...
async def payment_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve":
        await db_set_user_plan(int(order["user_id"]), order["plan"], int(order["days"]))
        await db_mark_payment_order_status(order_id, "manual_approved", note=f"approved by admin {uid}")
        try:
